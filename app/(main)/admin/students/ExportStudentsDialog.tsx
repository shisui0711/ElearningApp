...
import { ExcelField, exportToExcel, studentFields } from "@/lib/excel";
...
type ExportStudentsDialogProps = {
  open: boolean;
  onClose: () => void;
  students: any[];
};
...
export default function ExportStudentsDialog({
  ...
  // Map student data to include user properties at top level
  const prepareStudentData = (students: any[]) => {
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      const preparedData = prepareStudentData(students);
      exportToExcel(preparedData, selectedFields, 'danh-sach-sinh-vien');
      onClose();
    } catch (error) {
